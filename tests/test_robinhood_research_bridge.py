...
def _equity_record(collected_at: str = "2026-07-10T20:55:00Z") -> dict:
    return {
        ...
        "fundamentals": {
            "symbol": "AAPL", "market_cap": "3200000000000", "pe_ratio": "31.5",
            "pb_ratio": "42.0", "sector": "Electronic Technology",
            "industry": "Telecommunications Equipment", "volume": "50000000",
            "average_volume_30_days": "48000000", "high_52_weeks": "220",
            "low_52_weeks": "165", "float": "15000000000",
        },
        "earnings": [{
        ...
        "equity_history": {
            "symbol": "AAPL",
            "bars": [
                {"begins_at": f"2026-06-{day:02d}T00:00:00Z", "close_price": str(180 + day), "interpolated": False}
...
def _option_record() -> dict:
    ...
    record.update({
        ...
        "option_request": {
            "asset": "option", "ticker": "AAPL", "expiry": "2026-12-18",
            "side": "call", "strike": 220,
        },
        "option_contracts": [{
            "instrument": {
                "id": "opt-aapl-220", "chain_symbol": "AAPL",
                "expiration_date": "2026-12-18", "strike_price": "220.0000",
                "type": "call", "state": "active", "tradability": "tradable",
                "sellout_datetime": "2026-12-18T19:30:00Z",
            },
            "quote": {
                "quote": {
                    "instrument_id": "opt-aapl-220", "mark_price": "9.00",
                    "bid_price": "8.90", "ask_price": "9.10", "bid_size": 12,
                    "ask_size": 8, "volume": 1400, "open_interest": 22000,
                    "implied_volatility": "0.31", "delta": "0.52", "gamma": "0.015",
                    "theta": "-0.04", "vega": "0.22", "chance_of_profit_long": "0.44",
                    "break_even_price": "229.00", "low_fill_rate_buy_price": "8.95",
                    "high_fill_rate_buy_price": "9.08", "updated_at": "2026-07-10T20:54:30Z",
                },
...
def test_fresh_cache_satisfies_matching_request():
    with tempfile.TemporaryDirectory() as temp_dir:
        data_dir = Path(temp_dir)
        bridge._write_json(data_dir / bridge.SNAPSHOT_PATH.name, {
            "schema": bridge.SNAPSHOT_SCHEMA, "records": [_equity_record()],
        })
...
def test_option_normalization_preserves_exact_liquidity_and_greeks():
    ...
    row = bridge.flatten_option_contract(
        record["option_contracts"][0], record["collected_at"], asof=ASOF,
    )
...
def test_lookup_sections_filters_to_exact_requested_contract():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        sections = bridge.lookup_sections(
            "AAPL",
            {"asset": "option", "ticker": "AAPL", "expiry": "2026-12-18", "side": "call", "strike": 220},
            data_dir=data_dir,
            asof=ASOF,
        )
...
def test_snapshot_merge_preserves_existing_records():
    ...
    assert {row["request_id"] for row in merged["records"]} == {
        "equity:AAPL", "option:AAPL|2026-12-18|call|220",
    }
