...
from backtest import exit_rules, share_positions  # noqa: E402
...
def test_share_position_closes_on_stop():
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            share_positions.add_new_share_signals(
                pd.DataFrame([{"ticker": "AAA", "spot": 10, "stop_pct": -0.1, "target_pct": 0.2}]),
                asof,
            )
...
def test_share_position_keeps_open_on_reprice_failure():
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            asof = datetime.now(UTC)
            share_positions.add_new_share_signals(
                pd.DataFrame([{"ticker": "AAA", "spot": 10}]), asof
            )
...
def test_share_position_backfills_missing_entry_price():
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            added = share_positions.add_new_share_signals(
                pd.DataFrame(
                    [
                        {
                            "ticker": "AAA",
