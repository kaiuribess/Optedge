...
from backtest import positions  # noqa: E402
...
def test_option_position_adds_trade_row():
    with tempfile.TemporaryDirectory() as td:
        old_data, old_open, old_closed = (
            positions.DATA_DIR,
            positions.OPEN_FILE,
            positions.CLOSED_FILE,
        )
        _patch_files(td)
        try:
            ...
            positions.DATA_DIR, positions.OPEN_FILE, positions.CLOSED_FILE = (
                old_data,
                old_open,
                old_closed,
            )
...
def test_option_position_respects_reentry_cooldown():
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            positions.CLOSED_FILE.write_text(
                json.dumps(
                    [
                        {
                            "ticker": "AAPL",
...
def test_option_position_stays_open_through_expiration_date():
    with tempfile.TemporaryDirectory() as td:
        ...
        _patch_files(td)
        try:
            asof = datetime(2026, 6, 18, 20, 0, tzinfo=UTC)
            positions.OPEN_FILE.write_text(
                json.dumps(
                    [
                        {
                            "ticker": "AAPL",
...
def test_option_position_expiry_uses_historical_underlying_close():
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            assert (
                closed_rows[0]["expiry_close_price_source"]
