...
5. In the Robinhood connection panel, choose **Live-check best option**. Optedge keeps the queue order unchanged and reads only `orders[0]`; it does not hunt for a different contract after seeing live prices. The check first proves that the candidate is identical in the fresh cycle and queue, follows bounded Robinhood pages, and requires exactly one active, tradable, standard 100-share contract for the planned underlying. Robinhood's current chain and quote tools omit `next` on terminal responses; Optedge accepts that only when the live schema forbids cursor input. The instrument tool's cursorless terminal response is accepted only for the frozen chain/expiry/strike/type/state/tradability query, at most four rows, and rows that all match that exact scope. Any broader, mismatched, cyclic, or ambiguous page still blocks. The check then requires a quote no older than 120 seconds, an ask no higher than the frozen research cap, the profile spread ceiling, and the profile liquidity and delta rules. A passed quote is market evidence only: it cannot clear a blocked local entry gate. The persisted artifact contains sanitized public contract/quote evidence, not credentials, account data, an order, or broker authority. While the check is fresh, **Capture checked evidence** can append exactly one source-bound paper signal without making another broker call.
...
10. Stop at the preview. The current Optedge release has no placement API, so it cannot submit the order represented by the packet. Any later decision must be made deliberately in a Robinhood-supported surface after reviewing the current broker state; a preview is not a submission and a submission is not a fill.
