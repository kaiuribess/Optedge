...
[Quick start](#quick-start) · [Decision workflow](#decision-workflow) · [Edge Lab](#edge-lab) · [Dashboard](#dashboard) · [Robinhood review](#manual-robinhood-review) · [Project map](#project-layout) · [Documentation](#documentation)
...
6. Freezes forward-test metadata before outcomes are known and keeps current executable, current shadow, and legacy research evidence separate.
...
| Exact strategy, experiment, methodology, and evidence-policy provenance | Required |
...
- Exact candidate preservation from research card to planner. An incomplete option identity is blocked instead of being converted into a share plan.
...
A long call or put requires one same active, funded, Agentic-accessible Robinhood account with options level 2 or 3. If options level 2 was newly enabled, refresh and normalize the read-only broker snapshot before expecting the readiness panel to change. The permission only establishes account eligibility for supported long options; it does **not** validate the strategy, guarantee an order review, bypass buying-power or spread checks, enable multi-leg/short strategies, or authorize submission.
...
4. For an option, build a fresh research shortlist with `python scripts/export_robinhood_agentic_queue.py --account-budget 500`. The queue is not a review packet and cannot authorize a broker tool. It only supplies an exact candidate to Trade Desk when the contract remains in `manual_review_candidates` and identifies an equity/ETF underlying as `underlying_type=equity`.
5. In **Trade Desk**, load that exact candidate or enter a share plan, verify the account-equity/risk/allocation assumptions and proposed entry, stop, and target, then click **Calculate plan**. The gate requires current Edge Lab and validation evidence plus one same active account that satisfies portfolio value, permissions, conservative buying power, per-trade risk, and the total-open allocation check. It recomputes `current broker capital at risk + proposed capital at risk` against `min(planner equity, same-account live equity) x allocation fraction`; it never mixes capacity or exposure across accounts.
6. If the gate passes, copy the Robinhood review request into one connected task before its 10-minute expiry.
7. The connected task must refresh that exact account, positions, working orders, exact instrument, tradability, and live quote. The live instrument's `underlying_type` must exactly match the packet. It recomputes per-trade risk and total-open exposure against fresh same-account portfolio value, uses the smaller of explicit buying power and unleveraged buying power for order affordability, requires positive bid/ask values no older than 120 seconds, enforces the packet's numeric spread cap, and never raises the packet limit. It then calls the Robinhood review tool, shows the complete preview and alerts, and asks you to confirm that exact order.
...
V2 broker readiness uses only a positive `portfolio.total_value` plus both explicit `buying_power` and `unleveraged_buying_power`; it does not substitute equity aliases or cash. Reconciliation assigns each account a stable pseudonymous key and compares broker holdings only with local lifecycle rows explicitly marked as broker-linked. Ordinary Optedge research recommendations and Agentic paper rows remain visible as separate informational counts and do not create a live holding or reconciliation mismatch. Exact account, direction, aggregate quantity, and option-contract identity must agree, so same-nickname accounts, long/short differences, and quantity differences cannot be mistaken for a match.
...
| `risk/` | Research guardrails, same-account portfolio exposure controls, and review-only trade-plan packet construction. |
...
- Treat everything under `data/`, `logs/`, `telemetry/`, and `archive/` as private unless you have inspected and sanitized it.
...
Signals require human review. Performance depends on data quality, actual fills, spreads, slippage, liquidity, borrow and assignment behavior, regime changes, news shocks, earnings gaps, taxes, and sample size. Free providers can be delayed, incomplete, inconsistent, or unavailable. Model scores, backtests, forward tests, confidence intervals, and broker previews are evidence with uncertainty—not promises.
