...
- One short-lived manual Robinhood review packet you can copy or save as an inspection copy. The packet revalidates its semantic content, prompt, calculations, context, and expiry before display and can begin a broker preview; any later confirmation and submission occur outside the current Optedge release. It explicitly forbids batches, loops, scheduled tasks, repeated orders, and automatic retries.
...
4. For an option, build a fresh normal swing shortlist with `python scripts/export_robinhood_agentic_queue.py --account-budget 500`. The default lane remains `90+` DTE. To opt into LEAPS, run `python scripts/export_robinhood_agentic_queue.py --account-budget 500 --execution-profile leaps_swing --refresh-chain --chain-preset leaps`; that separate lane requires `365-900` DTE, explicit `option_leaps_swing` evidence, 3/5/10-session thesis reviews, and a 20-session maximum planned hold. A long expiry does not require a long hold. In either lane, the queue is not a packet or broker authority, the exact contract must occur once and identically in the fresh cycle and queue, and every no-execution control must remain intact. See [LEAPS Swing Profile](docs/LEAPS_SWING.md).
5. In the Robinhood connection panel, choose **Live-check best option**. Optedge keeps the queue order unchanged and reads only `orders[0]`; it does not hunt for a different contract after seeing live prices. The check first proves that the candidate is identical in the fresh cycle and queue, follows bounded Robinhood pages, and requires exactly one active, tradable, standard 100-share contract for the planned underlying. Robinhood's current chain and quote tools omit `next` on terminal responses; Optedge accepts that only when the live schema forbids cursor input. The instrument tool's cursorless terminal response is accepted only for the frozen chain/expiry/strike/type/state/tradability query, at most four rows, and rows that all match that exact scope. Any broader, mismatched, cyclic, or ambiguous page still blocks. The check then requires a quote no older than 120 seconds, an ask no higher than the frozen research cap, the profile spread ceiling, and the profile liquidity and delta rules. A passed quote is market evidence only: it cannot clear a blocked local entry gate. The persisted artifact contains sanitized public contract/quote evidence, not credentials, account data, an order, or broker authority.
6. In **Trade Desk**, load the checked option candidate or an exact fresh, actionable `top_shares_*.parquet` candidate, verify the account-equity/risk/allocation assumptions and proposed entry, stop, and target, then click **Calculate plan**. Free-form share inputs can still calculate local sizing, but they cannot produce a copyable Robinhood packet. An ordinary share scan preserves the last history-bar close and its market session, then derives deterministic entry, stop, and target geometry from that reference. The reference is not a live quote: its attestation may explicitly say `candidate_quote_available=false`, and the direct preview client or manual fallback task must still obtain a fresh bid/ask before review. The manual gate binds the share symbol, direction, geometry, size cap, price-reference provenance, source digest, row fingerprint, actionability, guard status, and artifact freshness so an arbitrary planner symbol cannot borrow another share's evidence. It also requires current Edge Lab and validation evidence plus one same active account that satisfies the chained drawdown interlock, portfolio value, permissions, conservative buying power, per-trade risk, and the total-open allocation check. It recomputes `current broker capital at risk + proposed capital at risk` against `min(planner equity, same-account live equity) x allocation fraction`; it never mixes capacity, loss history, or exposure across accounts.
7. If the gate passes, create one packet before its 10-minute expiry. The direct client contains an allowlisted broker-preview foundation; when that preview surface is not available in the current UI, copy the request into one separately connected Robinhood task as the manual fallback. Neither path can place through Optedge.
8. The direct client or fallback task must independently refresh the exact derived account identity, portfolio, positions, working orders, instrument, tradability, and live quote immediately before review. Every collection must reach an explicit final page. A failed, missing, stale, or ambiguous read blocks review. For an option, all matching chains and instruments must resolve to exactly one active buy-to-open standard contract with the planned underlying. The review uses a quote no older than 120 seconds and a spread no wider than the packet cap: at most `1%` for shares, `15%` for normal swing options, and `10%` for `leaps_swing`. The limit cannot rise. Only then may the review tool show the complete preview, disclosures, alerts, fees, collateral, and estimated cost.
9. Stop at the preview. The current Optedge release has no placement API, so it cannot submit the order represented by the packet. Any later decision must be made deliberately in a Robinhood-supported surface after reviewing the current broker state; a preview is not a submission and a submission is not a fill.

The broker boundary is manual and on demand. Optedge does not create a recurring Codex task or automatic trade loop. The Robinhood queue and local auto-paper script are research/paper artifacts: a queue row may be loaded as an exact Trade Desk input, but neither artifact is a review packet or broker authorization. Only a newly calculated, unexpired Trade Desk packet can begin a review. Packet v2 includes a canonical semantic SHA-256 digest and a prompt digest, then re-runs its structural, arithmetic, context, and expiry checks before rendering. Those digests detect accidental or deliberate local modification; they are not a signature, authentication, or standalone authority to call a broker tool. A saved packet is inspection-only and must be rebuilt if expired or changed.

Each packet also binds one deterministic UUIDv5 Robinhood `ref_id` from its packet ID using namespace `60d21b6d-517b-5d2d-b303-6ce65ff6a725`. Rebuilding identical packet content produces the same logical-order ID; creating genuinely new content produces a new one. The identifier is audit context, not placement authority. Optedge never automatically retries and the current release exposes no placement call.
