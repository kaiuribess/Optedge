...
> [!IMPORTANT]
> Optedge is research and decision-support software. It does not guarantee returns, does not autonomously trade, and does not make an order safe merely because Robinhood accepts it. The local application has no broker-order endpoint. Any Robinhood action remains an external, short-lived, review-before-send workflow that requires explicit user confirmation.
...
| Robinhood connection | External, approval-gated review and optional one-time submission; never an unattended loop. |
...
Discover candidates
  ...
  -> explicitly confirm or reject that unchanged order
  -> monitor broker state and manage the position in Robinhood
```
...
- One short-lived manual Robinhood review packet you can copy or save as an inspection copy. The packet revalidates its semantic content, prompt, calculations, context, and expiry before display; it still requires live broker review and exact confirmation and explicitly forbids batches, loops, scheduled tasks, repeated orders, and automatic retries.
...
A long call or put requires one same active, funded, Agentic-accessible Robinhood account with options level 2 or 3. If options level 2 was newly enabled, refresh and explicitly normalize the read-only broker snapshot before expecting the readiness panel to change. The permission only establishes account eligibility for supported long options; it does **not** validate the strategy, guarantee an order review, bypass model, evidence, drawdown, buying-power, overlap, or spread checks, enable multi-leg/short strategies, or authorize submission.
...
3. Refresh the read-only broker snapshot and normalize it with `python scripts/normalize_robinhood_broker_snapshot.py`. Use the account-scoped `optedge_robinhood_mcp_read_bundle_v2` capture format in [docs/THIRD_PARTY_FORWARD_TESTING.md](docs/THIRD_PARTY_FORWARD_TESTING.md): preserve the exact decoded `data` envelopes and collection lists; include portfolio, equity-position, option-position, equity-order, and option-order reads for every returned account under the exact request `account_number`; keep those account-scoped reads out of the top level; finish every paginated page with an explicit `data.next` key and the final value set to `null`; preserve each follow-up request cursor when the preceding `next` URL exposes one; and join every open option `option_id` to `get_option_instruments`. Missing/wrong-shaped sections, malformed collection rows, incomplete or unlinked pagination, unscoped reads, missing option instruments, or a missing source timestamp are blocked rather than guessed. Manual review accepts only `optedge_robinhood_broker_snapshot_v1` normalized from this v2 bundle; legacy snapshots remain display-only. Its broker source time must be no more than 45 minutes old; a new `normalized_at` value cannot renew stale broker data. `--dry-run` writes neither the snapshot nor the equity ledger. For manual review, account identity follows `optedge_robinhood_account_key_derivation_v1`: trim the exact `get_accounts.account_number`, compute SHA-256 over UTF-8 `optedge-robinhood-account-v1|<trimmed account number>`, take the first 16 lowercase hexadecimal characters, and prefix `acct_`. The normalized snapshot and review packet never persist the raw number; the ignored raw capture necessarily contains it and must remain private. A `...last4` mask is display-only and is not unique.
4. For an option, build a fresh research shortlist with `python scripts/export_robinhood_agentic_queue.py --account-budget 500`. The queue is not a review packet and cannot authorize a broker tool. Trade Desk requires the exact contract to occur once in both the fresh cycle's `manual_review_candidates` and the fresh queue's `orders`, with identical canonical row content. The attestation binds full SHA-256 digests for the cycle, queue, and row plus the row digest's first 24 hexadecimal characters as its fingerprint. DTE must canonically equal `expiry date - cycle UTC date` and be at least `90`. The cycle must say `auto_submit_allowed=false` and `does_not_place_orders=true`; the queue must say `does_not_place_orders=true`, `execution_enabled=false`, and `max_orders_to_submit=0`. These are research controls, not broker authority.
5. In **Trade Desk**, load an exact option-queue candidate or an exact fresh, actionable `top_shares_*.parquet` candidate, verify the account-equity/risk/allocation assumptions and proposed entry, stop, and target, then click **Calculate plan**. Free-form share inputs can still calculate local sizing, but they cannot produce a copyable Robinhood packet. An ordinary share scan preserves the last history-bar close and its market session, then derives deterministic entry, stop, and target geometry from that reference. The reference is not a live quote: its attestation may explicitly say `candidate_quote_available=false`, and the connected Robinhood task must still obtain a fresh bid/ask before review. The manual gate binds the share symbol, direction, geometry, size cap, price-reference provenance, source digest, row fingerprint, actionability, guard status, and artifact freshness so an arbitrary planner symbol cannot borrow another share's evidence. It also requires current Edge Lab and validation evidence plus one same active account that satisfies the chained drawdown interlock, portfolio value, permissions, conservative buying power, per-trade risk, and the total-open allocation check. It recomputes `current broker capital at risk + proposed capital at risk` against `min(planner equity, same-account live equity) x allocation fraction`; it never mixes capacity, loss history, or exposure across accounts.
6. If the gate passes, copy the Robinhood review request into one connected task before its 10-minute expiry.
7. The connected task must refresh the exact derived account identity, portfolio, positions, working orders, instrument, tradability, and live quote. Every equity/option position and order read must follow `data.next`/cursor pages to an explicit `null`; option review must also finish every chain and instrument page. A failed, missing, or unlinked page blocks review. It fetches a fresh equity quote for every held share symbol and a fresh option quote for every held `option_id` before computing total-open risk; missing or stale held-position prices block. A recent matching filled opening order also blocks until its corresponding position is visible, so broker-feed lag cannot be mistaken for permission to submit again. The live instrument's `underlying_type` must exactly match the packet. For an option, the task enumerates every chain whose `expiration_dates` contains the planned expiry, queries each `chain_id` with the exact type/strike/expiry filters, and requires exactly one active buy-to-open tradable instrument across all results; its nonnumeric `chain_symbol` must equal the underlying. The unique linked chain must have `id == instrument.chain_id`, the same symbol, `can_open_position=true`, multiplier `100`, `cash_component=null`, and the exact equity in `underlying_instruments`. Missing or ambiguous proof, or any instrument, chain, or preview indication of a nonstandard deliverable, blocks review. A `100` multiplier by itself is not proof of a standard contract. The task recomputes per-trade risk and total-open exposure against fresh same-account portfolio value, uses the smaller of explicit buying power and unleveraged buying power for affordability, and enforces positive ordered bid/ask values no older than 120 seconds. The live spread cannot exceed the hard `1%` share cap or `15%` option cap; an option candidate may be stricter. The limit may never rise. It then calls the Robinhood review tool, shows the complete preview and alerts, and asks you to confirm that exact order.
8. Immediately after that exact confirmation, the connected task must re-read every page of the selected account and portfolio, equity and option positions, equity and option orders, exact live quote, exact instrument, and—when applicable—every page of the complete all-expiry-chain proof. It refreshes every held-position quote again, re-checks recent unmatched fills, recomputes every risk, duplicate, permission, buying-power, quote, spread, tradability, tick, and deliverable gate, and re-checks the packet's 10-minute expiry. Any change, incomplete page, missing/stale held-position quote, failure, ambiguity, or expired packet aborts placement and requires a new preview and confirmation. Only an unchanged, unexpired state allows the one matching placement call with unchanged fields and the packet's existing deterministic reference ID. A submitted order is not treated as filled until Robinhood reports its broker state.

The broker handoff is manual and on-demand. Optedge does not create a recurring Codex task or automatic trade loop. The Robinhood queue and local auto-paper script are research/paper artifacts: a queue row may be loaded as an exact Trade Desk input, but neither artifact is a review packet or broker authorization. Only a newly calculated, unexpired Trade Desk packet can begin the separate connected review flow. Packet v2 includes a canonical semantic SHA-256 digest and a prompt digest, then re-runs its structural, arithmetic, context, and expiry checks before rendering. Those digests detect accidental or deliberate local modification; they are not a signature, authentication, or standalone authority to call a broker tool. A saved packet is inspection-only and must be rebuilt if expired or changed.

Each packet also binds one deterministic UUIDv5 Robinhood `ref_id` from its packet ID using namespace `60d21b6d-517b-5d2d-b303-6ce65ff6a725`. Rebuilding identical packet content produces the same logical-order ID; creating a genuinely new packet produces a new one. Optedge never automatically retries. If a user deliberately retries the same logical order after checking broker order state and obtaining the required review/confirmation, the task must reuse that exact `ref_id` rather than create a second logical order.
...
- Options chains and price history through a layered provider path, with optional read-only Robinhood/Codex caches for exact contract bars and interactive exact-contract quote checks.
