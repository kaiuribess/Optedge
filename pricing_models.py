...
from utils import bs_delta, bs_implied_vol, bs_price  # noqa: F401
...
def bs_price_vec(
    ...
    d1 = np.where(
        bad, 0.0, (np.log(np.where(bad, 1.0, S / K)) + (r - q + 0.5 * sigma**2) * T) / sigT
    )
    d2 = d1 - sigT
    ...
    intrinsic = np.where(call_mask, np.maximum(0.0, S - K), np.maximum(0.0, K - S))
...
def crr_price_vec(
    ...
    log_ST_term = (
        np.log(np.where(S > 0, S, 1.0))[:, None]
    ...
    K_b = K[:, None]
    payoff = np.where(
        call_mask[:, None], np.maximum(ST_term - K_b, 0.0), np.maximum(K_b - ST_term, 0.0)
    )
    ...
    for n in range(steps - 1, -1, -1):
        ...
        # Continuation value
        cont = disc[:, None] * (
            p[:, None] * vals[:, : n + 1] + (1.0 - p[:, None]) * vals[:, 1 : n + 2]
        )
        # American early-exercise
        ex = np.where(call_mask[:, None], np.maximum(ST_n - K_b, 0.0), np.maximum(K_b - ST_n, 0.0))
...
def _bivariate_cdf_vec(a: np.ndarray, b: np.ndarray, rho: np.ndarray) -> np.ndarray:
    ...
    if m4.any():
        out[m4] = (
            norm.cdf(a[m4]) + norm.cdf(b[m4]) - 1 + _bivariate_cdf_vec(-a[m4], -b[m4], rho[m4])
        )
...
def bjs_price_vec(S, K, T, r, sigma, q, call_mask) -> np.ndarray:
    ...
    intrinsic = np.where(call_mask, np.maximum(S - K, 0.0), np.maximum(K - S, 0.0))
    bs_floor = bs_price_vec(
        S, K, T, r if np.isscalar(r) else float(np.mean(r)), sigma, q, call_mask
    )
    # Use BS where BJS produces an unreasonably low value
    fallback_mask = (out < intrinsic) | ((bs_floor > 0) & (out < bs_floor * 0.5))
...
def _heston_char_fn(u, T, r, q, kappa, theta, rho, v0, xi, S0):
    ...
    exp_dT = np.exp(-d * T_b)
    C = (r - q) * i * u_b * T_b + kappa * theta / xi**2 * (
        (kappa - rho * xi * i * u_b - d) * T_b - 2 * np.log((1 - g * exp_dT) / (1 - g))
    )
    D = (kappa - rho * xi * i * u_b - d) / xi**2 * (1 - exp_dT) / (1 - g * exp_dT)
    return np.exp(C + D * v0 + i * u_b * np.log(S0)[:, None])


def heston_price_vec(
    S,
    K,
    T,
    r,
    ...
    def _P(b: float, sign_alpha: float):
        ...
        d = np.sqrt((rho * xi * i * u_c - b) ** 2 - xi**2 * (2 * sign_alpha * i * u_c - u_c**2))
        ...
        C_term = (r_scalar - q_b) * i * u_b * T_b + kappa * theta / xi**2 * (
            (b_minus_ru - d_b) * T_b - 2 * np.log((1 - g_b * exp_dT) / (1 - g_b))
        )
        D_term = (b_minus_ru - d_b) / xi**2 * (1 - exp_dT) / (1 - g_b * exp_dT)
        f = np.exp(C_term + D_term * v0 + i * u_b * np.log(Ssafe)[:, None])
        integrand = (np.exp(-i * u_b * np.log(Ksafe)[:, None]) * f / (i * u_b)).real  # (N, n_quad)
    ...
    intrinsic = np.where(call_mask, np.maximum(Ssafe - Ksafe, 0.0), np.maximum(Ksafe - Ssafe, 0.0))
...
def ensemble_theo(per_model: dict[str, float], weights: dict[str, float] | None = None) -> float:
    if not per_model:
        return float("nan")
...
def ensemble_theo_vec(
    per_model: dict[str, np.ndarray], weights: dict[str, float] | None = None
) -> np.ndarray:
...
def all_models_vec(
    ...
    q,
    call_mask,
    cboe_theo: np.ndarray | None = None,
    crr_steps: int = 80,
...
def all_models(
    ...
    return {k: float(v[0]) for k, v in res.items() if v is not None and np.isfinite(v[0])}
...
if __name__ == "__main__":
    ...
    for fn, name, kwargs in [
        ...
        print(
            f"{name:18}: {dt:6.1f}ms for {N} contracts  "
