...
def record(
    timings: dict[str, dict[str, Any]], empty_engines: list[dict[str, Any]] | None = None
) -> dict[str, Any]:
...
def summarize(rows: list[dict[str, Any]] | None = None, window: int = 20) -> dict[str, Any]:
    ...
    if not rows:
        return {"generated_at": datetime.now(UTC).isoformat(), "engines": []}
    ...
    for name, sub in df.groupby("engine"):
        ...
        score = max(
            ...
            min(
                100.0,
                100.0
                * (0.55 * ok_rate + 0.35 * hit_rate + 0.10 * (1.0 - min(avg_elapsed / 300.0, 1.0))),
            ),
        )
        engines.append(
            {
