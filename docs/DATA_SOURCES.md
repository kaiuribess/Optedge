...
| Robinhood market research | Optional allowlisted direct official MCP reads or bounded manual request/cache files for equities, exact options, and history | The upstream timestamp controls freshness. The ten-ticker discovery lane enumerates Robinhood-returned 90-900 DTE instruments, exact-quotes them, applies hard quality filters, prices every survivor, and rechecks up to three finalists per ticker; it never promotes a row. The stricter finalist gate resolves only unchanged execution-queue candidates through bounded chain/instrument pages and a live quote. Both results expire after 120 seconds. Exact quotes and trade bars are market observations, not fills, authorizations, or proof Optedge traded. Research artifacts deliberately exclude raw accounts and orders. |
| Robinhood account state | One explicit direct complete account read or a manually supplied account-scoped bundle, normalized into a pseudonymous broker snapshot | Direct sync keeps raw account identifiers in memory, proves bounded pagination, and persists only redacted state. The snapshot can become stale immediately and never authorizes an order; manual or guarded placement independently re-reads the selected live account. |
