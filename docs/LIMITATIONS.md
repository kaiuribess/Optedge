...
- The current release exposes connection, allowlisted account reads, and a broker-preview client foundation, but no placement API. A preview can still become stale immediately and does not make later broker calls atomic; any actual order decision must be reviewed again in a Robinhood-supported surface.
