...
14. One user-triggered ten-ticker research scan starts with normal Optedge-ranked underlyings, reads a fresh Robinhood underlying quote, enumerates the bounded 90-900 DTE instruments returned by Robinhood, exact-quotes them in bounded batches, applies cheap liquidity/delta/premium filters, and runs every survivor through Black-Scholes, CRR, Bjerksund-Stensland, spread-cost, confidence-shrinkage, low-model-bound, thesis-alignment, event-risk, and theoretical profit-probability checks. It rechecks up to three finalists per ticker and explicitly lets No Trade win. A stale after-hours option quote can remain visible only as theoretical research; conservative-positive status additionally requires fresh option and underlying quotes plus a passed normal Optedge research guard. Its rows are never promoted to the execution queue automatically.
...
- `optedge/robinhood_finalist.py`: fail-closed full-chain option EV funnel for the research-only ten-ticker comparison plus the short-lived queued-candidate exact-quote gate; only the latter is cryptographically bound to the unchanged normal Optedge queue and cycle.
