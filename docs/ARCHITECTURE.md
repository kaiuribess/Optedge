...
14. One user-triggered finalist check preserves the normal queue order and inspects only `orders[0]` through bounded official Robinhood chain, instrument, and quote reads. It proves exact contract uniqueness and standard-contract metadata, enforces the frozen limit, 120-second quote freshness, profile spread/liquidity/delta rules, and binds the result to unchanged cycle and queue digests. It cannot clear a blocked local entry gate, preview an order, or search for a replacement contract.
15. If every gate passes, the local app emits one expiring review packet with one deterministic packet-scoped UUIDv5 `ref_id`. The direct official Robinhood MCP connection can perform one user-triggered allowlisted read or order review after browser OAuth; a separately connected Robinhood task remains a manual packet fallback. Both paths independently re-read broker state immediately before preview and stop at broker preview in the current release because Optedge exposes no placement API.
...
- `optedge/robinhood_connection.py`: bounded synchronous cockpit bridge to one private asynchronous MCP connection; no poller, retry loop, generic dispatcher, or placement method.
- `optedge/robinhood_finalist.py`: fail-closed exact top-option resolver and short-lived live-quote promotion gate, cryptographically bound to the unchanged normal Optedge queue and cycle.
