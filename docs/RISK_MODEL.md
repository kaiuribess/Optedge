...
The live quote gate is numeric: bid and ask must be positive, ask must be at least bid, quote timestamps must be no more than 120 seconds old, and `(ask - bid) / ((ask + bid) / 2)` must be no greater than the packet cap. These are hard maximums: `1%` for shares, `15%` for normal swing options, and `10%` for `leaps_swing`; configuration cannot raise them, while a candidate may be stricter. The review stops when any field or timestamp is missing or when the live ask exceeds the packet limit. It cannot raise the limit, and Optedge exposes no placement step after the preview.
...
The packet describes one logical entry idea at a time. It derives one Robinhood idempotency `ref_id` as UUIDv5 of the deterministic packet ID under namespace `60d21b6d-517b-5d2d-b303-6ce65ff6a725`. The identifier preserves one audit identity; it is not placement authority. Optedge prohibits batches, scheduled tasks, loops, automatic retries, and field changes during review. The current release exposes no placement API and stops at broker preview.

Every direct snapshot collection read follows `data.next` through a bounded, exact cursor chain and requires explicit completion before replacing local state. Every preview preflight must independently refresh the chosen account, positions, orders, exact instrument/chain, and quotes; a saved snapshot is readiness evidence, not a substitute for same-session broker state. Missing pages, stale marks, recent unmatched fills, ambiguity, or packet expiry blocks preview. The current release performs no placement step; any later confirmation and action must occur deliberately in a Robinhood-supported surface after a new state review.
