...
Every explicit, non-dry-run normalization of a valid broker snapshot appends one observation per eligible pseudonymous account to a durable local ledger. When the normalizer uses the repository's real `data/` directory, `OPTEDGE_STATE_DIR` selects the ledger directory explicitly; otherwise Windows uses `%LOCALAPPDATA%\Optedge\risk`, and Unix-like systems use `$XDG_STATE_HOME/optedge/risk` or the normal `~/.local/state/optedge/risk` fallback. An explicit custom or test data directory remains self-contained at `<data_dir>/robinhood_account_equity_ledgers/`.
...
## Manual Robinhood Review Packet
...
Options and shares both require exact candidate attestation. An option must occur exactly once in the fresh cycle's `manual_review_candidates` and exactly once in the fresh queue's `orders`; both canonical rows must be identical. The attestation freezes the full cycle and queue SHA-256 digests, the canonical row digest, and its first 24 hexadecimal characters as the candidate fingerprint. It recomputes integer DTE as `expiry date - cycle UTC date` and requires at least 90 days. The cycle must keep new entries conditional on later live checks while declaring no placement and no auto-submit; the queue must declare no placement, `execution_enabled=false`, and `max_orders_to_submit=0`. Any duplicate, mismatch, stale artifact, altered digest, shorter DTE, or execution-enabled control blocks.
...
The live quote gate is numeric: bid and ask must be positive, ask must be at least bid, quote timestamps must be no more than 120 seconds old, and `(ask - bid) / ((ask + bid) / 2)` must be no greater than the packet cap. These are hard maximums: `1%` for shares and `15%` for options; configuration cannot raise them, while an option candidate may be stricter. The task stops when any field or timestamp is missing or when the live ask exceeds the packet limit. It cannot raise the limit or place anything until the user confirms the exact broker preview.
...
The packet supports one logical order at a time. It derives one Robinhood idempotency `ref_id` as UUIDv5 of the deterministic packet ID under namespace `60d21b6d-517b-5d2d-b303-6ce65ff6a725`. The same packet therefore keeps the same logical-order identity. Optedge prohibits batches, scheduled tasks, loops, automatic retries, and field changes between review and placement. If the user deliberately retries the same logical order after checking broker state and completing the required review/confirmation, the task must reuse that exact `ref_id`; it may not manufacture a second identity.

Every live collection read follows all `data.next`/cursor pages to explicit completion. Every held share symbol and option ID gets a fresh quote before total-open exposure is computed; missing or stale marks block. A recent matching filled opening order remains a duplicate blocker until the corresponding position is visible. Immediately after the user confirms the exact preview, the connected task repeats every page of the selected account/portfolio, equity and option positions, equity and option orders, exact quote, exact instrument, and, for options, the complete all-expiry-chain proof, then refreshes every held-position quote again. It recomputes every gate and revalidates the packet's 10-minute expiry. Any relevant change, incomplete page, missing/stale held-position price, failure, ambiguity, or expiry aborts the place call and requires a new preview and confirmation; only an unchanged, unexpired re-read permits one matching place call. If placement status is uncertain, broker orders must be queried before any further action.
