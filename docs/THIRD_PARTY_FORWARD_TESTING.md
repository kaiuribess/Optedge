...
- `agentic_allowed=true` means Robinhood may expose order tools for that account; Optedge still exposes no placement method and the flag does not authorize an order.
...
- The cockpit's **Robinhood MCP capability map** separates broker-advertised tool support, current account status, and Optedge's stricter local policy. Placement remains unavailable through Optedge even when Robinhood advertises a write tool.
...
The project includes a direct official MCP OAuth/read/preview foundation, but no placement API. OAuth material stays in the OS credential vault and is not a project credential file. Exports remain handoff files for paper tracking or a separate approval-gated review; they are not broker authority.
