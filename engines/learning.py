...
def save_weights(
    ...
    decay_flags: list[str] | None = None,
) -> Path:
...
def refit_bucket(
    ...
    if n >= min_lasso:
        ...
        try:
            ...
            coefs = dict(zip(factor_names, model.coef_.astype(float), strict=False))
    ...
    if n >= min_ic:
        ...
        for fn in priors.keys():
            ...
            cap = max(0.02, 2.0 * abs(priors.get(fn, 0.0)))
            blended[fn] = (
                float(np.sign(blended[fn]) * min(abs(blended[fn]), cap))
                if blended[fn] != 0
                else priors[fn]
            )

        save_weights(
            bucket, blended, source=mode, n_samples=n, factor_ic=factor_ic, decay_flags=decay_flags
        )
