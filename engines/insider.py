...
def _form4_filings(submissions: dict, cutoff: datetime) -> List[Dict[str, Any]]:
    ...
    out = []
    for f, acc, dt, prim in zip(forms, accs, dates, primaries):
        if f != "4":
            continue
        try:
            filed = datetime.strptime(dt, "%Y-%m-%d").replace(tzinfo=timezone.utc)
        except Exception:
            continue
        if filed < cutoff:
            continue
...
def _score(rows: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    if not rows:
        return {"insider_score": 0.0, "buys_value": 0.0, "sells_value": 0.0, "n_buys": 0, "n_sells": 0}
    buy_val = 0.0
    sell_val = 0.0
    n_buy = 0
    n_sell = 0
    weighted_buy = 0.0
    for r in rows:
        is_priority = r["is_officer"] or r["is_director"] or r["is_10pct"] or any(
            kw in r["title"] for kw in INSIDER_PRIORITY_TITLES
        )
...
def _process_ticker(t: str, cik_map: Dict[str, str], cutoff: datetime,
    ...
    if fast_mode:
        ...
        s = {"ticker": t, "insider_score": 0.0, "buys_value": 0, "sells_value": 0,
             "n_buys": 0, "n_sells": 0, "n_form4": n}
...
def _fetch_finnhub_insider_sentiment(ticker: str) -> Dict[str, float]:
    ...
    data = fh.get("/stock/insider-sentiment",
...
def run(universe: List[str], max_workers: int = None, fast_mode: bool = None,
        finnhub_top_n: int = 50) -> pd.DataFrame:
    ...
    out = []
    completed = 0
    log.info("processing %d tickers (parallel, %d workers, fast=%s, cap=%d filings/ticker)",
             len(universe), workers, fast_mode, INSIDER_MAX_FILINGS_PER_TICKER)
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futures = {ex.submit(_process_ticker, t, cik_map, cutoff, fast_mode): t
                   for t in universe}
        for fut in as_completed(futures):
            ...
            try:
                ...
                out.append({"ticker": t, "insider_score": 0.0, "buys_value": 0,
                            "sells_value": 0, "n_buys": 0, "n_sells": 0, "n_form4": 0})
            completed += 1
    ...
    if finnhub_data and not df.empty:
        df["finnhub_mspr"] = df["ticker"].map(lambda t: finnhub_data.get(t, {}).get("finnhub_mspr", 0))
        df["finnhub_change"] = df["ticker"].map(lambda t: finnhub_data.get(t, {}).get("finnhub_change", 0))
