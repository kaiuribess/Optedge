...
def _earnings_window(days_out: int) -> str:
    return (datetime.now(UTC) + timedelta(days=days_out)).date().isoformat()
...
def _synthesize_chain(
    ticker: str, asof: datetime, rng: random.Random, r: float = 0.045
) -> pd.DataFrame:
    ...
    for exp in _next_three_expiries(asof):
        T_days = (datetime.fromisoformat(exp).replace(tzinfo=UTC) - asof).days
        T = T_days / 365.25
        ...
        for K in _strike_grid(spot):
            ...
            for side in ("call", "put"):
                ...
                liq_factor = (
                    0.5 if ticker in ("SPY", "QQQ", "AAPL", "NVDA", "TSLA", "MSFT", "AMD") else 1.0
                )
                ...
                base_oi = (
                    ...
                    if ticker in ("AAPL", "NVDA", "TSLA", "MSFT", "AMD")
                    else 1200
                )
...
def synthetic_mispricing(
    universe: list[str], asof: datetime, seed: int = 42
) -> dict[str, pd.DataFrame]:
    """Run the same enrichment + filter pipeline as engines.mispricing.run, on synth chains."""
    from engines.mispricing import _apply_filters, _enrich_chain, _per_ticker_summary
...
def synthetic_fundamentals(universe: list[str], seed: int = 42) -> pd.DataFrame:
    ...
    for t in universe:
        ...
        d["fund_score"] = _fund_score(
            d.get("rev_growth"), d.get("op_margin"), d.get("pe"), d.get("ps"), d.get("fcf_yield")
        )
        # Default fwd_pe
        ...
        rows.append(
            {
                ...
                "buys_value": round(buys_value, 0),
                "sells_value": round(sells_value, 0),
                "n_buys": n_buys,
                "n_sells": n_sells,
            }
        )
...
def synthetic_news(universe: list[str], seed: int = 42) -> pd.DataFrame:
    ...
    for t in universe:
        if t in hot:
            n_24 = rng.randint(2, 12)
            n_7 = rng.randint(15, 60)
            sent_24 = rng.uniform(-0.05, 0.45)
            sent_7 = rng.uniform(-0.10, 0.30)
            head = rng.choice(sample_pos)
        elif t in cold:
            n_24 = rng.randint(1, 5)
            n_7 = rng.randint(5, 25)
            sent_24 = rng.uniform(-0.45, 0.0)
            sent_7 = rng.uniform(-0.35, 0.10)
            head = rng.choice(sample_neg)
        else:
            n_24 = rng.randint(0, 3)
            n_7 = rng.randint(0, 12)
            sent_24 = rng.uniform(-0.10, 0.10)
            sent_7 = rng.uniform(-0.10, 0.10)
        ...
        rows.append(
            {
                ...
                "peg_ratio": round(pe / (rev_growth * 100), 2)
...
def synthetic_congress(universe: list[str], seed: int = 42) -> pd.DataFrame:
    ...
    for t in universe:
        ...
        n_buys = (
            rng.choices([0, 1, 2, 3, 5], weights=[5, 3, 2, 1, 1])[0]
            if t in hot
...
def synthetic_analyst(universe: list[str], seed: int = 42) -> pd.DataFrame:
    ...
    for t in universe[:120]:  # cap to top 120
        ...
        momentum = (
            rng.choice([-3, -1, 0, 0, 0, 1, 2, 4])
            if t in bullish
            else rng.choice([-4, -2, -1, 0, 0, 1])
            if t in bearish
            else rng.choice([-1, 0, 0, 0, 1])
        )
